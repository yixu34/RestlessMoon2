...
float4x3    boneMatrixArray[MAX_MATRICES] : WORLDMATRIXARRAY;
...
VS_OUTPUT VShade(VS_INPUT i, uniform int NumBones)
{
    ...
    for (int iBone = 0; iBone < NumBones-1; iBone++)
    {
        ...
        Pos += mul(i.Pos, boneMatrixArray[IndexArray[iBone]]) * BlendWeightsArray[iBone];
        Normal += mul(i.Normal, boneMatrixArray[IndexArray[iBone]]) * BlendWeightsArray[iBone];
    }
    LastWeight = 1.0f - LastWeight; 

    // Now that we have the calculated weight, add in the final influence
    Pos += (mul(i.Pos, boneMatrixArray[IndexArray[NumBones-1]]) * LastWeight);
    Normal += (mul(i.Normal, boneMatrixArray[IndexArray[NumBones-1]]) * LastWeight); 
